...
for elem in table_cont:
    ...
    if elem[3].text != " ":
        ...
        country_dpt[country.lower()] = dpt(deaths, total)
    else:
        country_dpt[country.lower()] = 0
...
def main():
    ...
    if choice == "show_all":
        ...
        for i in range(10):
            if dpt_list[i][0].title() in country_names:
                print(dpt_list[i][0].title() + ":", str(dpt_list[i][1]) + "%")
            else:
                print(dpt_list[i][0].upper() + ":", str(dpt_list[i][1]) + "%")
        ...
        print("\nCovid-19 Deaths per Total cases:\n")
        print("Total:", str(total_dpt) + "%\n")
        if choice.title() in country_names:
            print(choice.title() + ":", str(country_dpt[choice]) + "%\n")
        else:
            print(choice.upper() + ":", str(country_dpt[choice]) + "%\n")
        print(
